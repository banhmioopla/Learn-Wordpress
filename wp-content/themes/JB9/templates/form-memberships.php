      ...
      <div class="row">
         ...
         <div class="col-md-6 text-left box-desc">
            ...
            <ul class="fa-ul my-4 ml-4">
               <?php if(THEME_KEY == "ph"){ ?>
               <li class="lead">
                  <?php if(isset($csubscriptions['tokens'][$i])){ ?>
                  <strong><?php echo stripslashes($csubscriptions['tokens'][$i]); ?></strong>
                  <?php echo __("tokens now","premiumpress"); ?> <?php echo __("plus","premiumpress"); ?> 
                  <?php } ?>
                  <?php if(isset($csubscriptions['day_tokens'][$i])){ ?>
                  <strong><?php echo stripslashes($csubscriptions['day_tokens'][$i]); ?></strong> <?php echo __("more daily","premiumpress"); ?>
                  <?php } ?>
               </li>
               <?php }elseif(THEME_KEY == "da"){ ?>
               <li>
                  <?php if(isset($csubscriptions['access_gift'][$i]) && $csubscriptions['access_gift'][$i] == 1){  ?> 
                  <span class="fa fa-li"><i class="fa fa-check"></i></span> 
                  <?php }else{ ?>
                  <span class="fa-li"><i class="fa fa-times"></i></span>
                  <?php } ?>
                  <?php echo __("Send Virtual Gifts","premiumpress"); ?>
               </li>
               <li>
                  <?php if(isset($csubscriptions['access_messages'][$i]) && $csubscriptions['access_messages'][$i] == 1){  ?> 
                  <span class="fa fa-li"><i class="fa fa-check"></i></span> 
                  <?php }else{ ?>
                  <span class="fa-li"><i class="fa fa-times"></i></span>
                  <?php } ?>
                  <?php echo __("Send &amp; Receive Messages","premiumpress"); ?>
               </li>
               <li>
                  <?php if(isset($csubscriptions['access_chatroom'][$i]) && $csubscriptions['access_chatroom'][$i] == 1){  ?> 
                  <span class="fa fa-li"><i class="fa fa-check"></i></span> 
                  <?php }else{ ?>
                  <span class="fa-li"><i class="fa fa-times"></i></span>
                  <?php } ?>
                  <?php echo __("Chatroom Access","premiumpress"); ?>
               </li>
               <li>
                  <?php if(isset($csubscriptions['access_featured'][$i]) && $csubscriptions['access_featured'][$i] == 1){  ?> 
                  <span class="fa fa-li"><i class="fa fa-check"></i></span> 
                  <?php }else{ ?>
                  <span class="fa-li"><i class="fa fa-times"></i></span>
                  <?php } ?>
                  <?php echo __("Featured Profile","premiumpress"); ?>
               </li>
               <li>
                  <span class="fa fa-li"><i class="fa fa-check"></i></span> 
                  <?php echo __("Image &amp; Video Uploads","premiumpress"); ?>: <?php if(isset($csubscriptions['access_maxuploads'][$i]) && is_numeric($csubscriptions['access_maxuploads'][$i])){ echo $csubscriptions['access_maxuploads'][$i]; }else{ echo 0; }  ?> 
               </li>
               <?php }else{ ?>
               <?php if(defined('WLT_CREDITSYSTEM')){ ?>               
               <?php if(_ppt('tokens_system') != 1 && $csubscriptions['desc'][$i] == ""){ ?>                   
               <li>                   
                  <br /> <?php echo __("plus","premiumpress"); ?>  <strong><?php echo stripslashes($csubscriptions['tokens'][$i]); ?></strong>
                  <?php echo __("tokens now","premiumpress"); ?> <br /> <?php echo __("plus","premiumpress"); ?> 
                  <strong><?php echo stripslashes($csubscriptions['day_tokens'][$i]); ?></strong> <?php echo __("more daily","premiumpress"); ?>                   
               </li>
               <?php } ?>            
               <?php  } ?>
